...
void ANaveSubscriptor_2::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void ANaveSubscriptor_2::Tick(float DeltaTime)
{
...
void ANaveSubscriptor_2::EstablecerRadar(ARadar* _Radar)
{
	if (!_Radar)
	{
...
void ANaveSubscriptor_2::Actualizar(APublicadorObserver* _Publicador)
{
	MejorarSubscriptor();
}

void ANaveSubscriptor_2::MejorarSubscriptor()
{
	Energy = Radar->GetEnergia();
	if (Energy.Equals("25"))
	{
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, FString::Printf(TEXT("NaveSub2 Activar: SuperVelocidad y Armamento"), *Energy));
	...
	if (Energy.Equals("15"))
	{
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, FString::Printf(TEXT("NaveSub2 Activar: Escudo y Mejorar: Armamento"), *Energy));
		SArmamento = 1;
	...
	if (Energy.Equals("7"))
	{
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, FString::Printf(TEXT("NaveSub2 Mejorar: SuperVelocidad"), *Energy));
		MejorarSuperVelocidad();
	}
}

void ANaveSubscriptor_2::ActivarArmamento()
{
	if (Armamento == 1)
	{
...
void ANaveSubscriptor_2::MejoraArmamento(float DeltaTime)
{
	if (SArmamento == 1)
	{
		FVector PosNave = GetActorLocation();
...
void ANaveSubscriptor_2::ActivarSuperVelocidad()
{
	Velocidad = Velocidad * 2.0f;
}

void ANaveSubscriptor_2::MejorarSuperVelocidad()
{
	Velocidad = Velocidad * 2.0f;
}

void ANaveSubscriptor_2::ActivarEscudo()
{
	Escudo = 1;
}

void ANaveSubscriptor_2::MejorarImplementarEscudo()
{
	if (Escudo == 1)
	{
...
void ANaveSubscriptor_2::DestruirSubscripcion()
{
	Radar->DeSubscribirse(this);
}

void ANaveSubscriptor_2::Mover(float DeltaTime)
{
